...
"""
    Simulated dataset.
    @items, number of items (unique classes) in the population
    @fpr, false positive error rate
    @fnr, false negative error rate
    @workers, number of workers
    @dirty, proportion of the number of dirty items (ground truth)
    @recall, sampling rate (how many items does each worker sample without replacement from the items set)
    @err_skew, if True, then some items are easier/harder for workers to label correctly; fpr & fnr controls the average error rates.
    @priotized, if True, worker samples from (or are given) non-obvious items
    @eps, controls the randomization degree in priotization
    @hdirty, controls the proportion of the dirty heuristic pairs/items (non-obvious pairs/items)   
    @error_type, 0: flips label, 1: false positive (label becomes 1), 2: false negative 
"""
...
"""
    Address dataset. Crowdworkers are asked to identify malformed US home addresses. 
"""
def loadAddress():
...
"""
    Product dataset. Workers are asked to identify if postings from Amazon and Google refer to the same entity.
"""
...
"""
    Restaurant dataset. Workers are asked to identify duplicate restaurant entities (information) in CA.
"""
...
"""
    Jaccard similarity between two words.
"""
def jaccard(a,b):
